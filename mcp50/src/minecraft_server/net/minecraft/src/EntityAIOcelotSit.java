...
public class EntityAIOcelotSit extends EntityAIBase
{
    ...
    {
        return currentTick <= maxSittingTicks && field_52005_h <= 60 && isSittableBlock(theOcelot.worldObj, sitableBlockX, sitableBlockY, sitableBlockZ);
    }
    ...
    {
        theOcelot.getNavigator().tryMoveToXYZ((double)(float)sitableBlockX + 0.5D, sitableBlockY + 1, (double)(float)sitableBlockZ + 0.5D, field_50017_b);
        currentTick = 0;
        field_52005_h = 0;
        maxSittingTicks = theOcelot.getRNG().nextInt(theOcelot.getRNG().nextInt(1200) + 1200) + 1200;
        theOcelot.func_50021_C().setSitting(false);
    }
